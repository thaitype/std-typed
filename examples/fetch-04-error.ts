...
Std.runExit(async () => {
  ...
  for (const url of [
    ...
    match(result.into())
      .with({ _tag: "success" }, ({ value }) => console.log(`Fetch Result (url="${cleanedUrl}"): => ${JSON.stringify(value)}`))
      .with({ _tag: "failure", error: { _tag: "FetchError" } }, error => console.error(`Failed to fetch (url="${cleanedUrl}"): => FetchError ${error.error.stack}`))
      .with({ _tag: "failure", error: { _tag: "InvalidJsonError" } }, error => console.error(`Invalid JSON (url="${cleanedUrl}"): => InvalidJsonError ${error.error.stack}`))
      .with({ _tag: "failure", error: { _tag: "RequestFailError" } }, error => console.error(`Request failed (url="${cleanedUrl}"): => RequestFailError ${error.error.stack}`))
      .exhaustive();
  }
});
